 ...
 */
int hsh(information_t *information, character **av)
{
	ssize_t r = 0;
	int builtin_ret = 0;
...
int find_builtin(information_t *information)
{
	...
	for (i = 0; builtintbl[i].type; i++)
		if (_string_varcmp(information->argv[0], builtintbl[i].type) == 0)
		{
...
void find_cmd(information_t *information)
{
	...
	path = find_path(information, _getenv_func(information, "PATH="), information->argv[0]);
	if (path)
	{
	...
	{
		if ((interactive(information) || _getenv_func(information, "PATH=")
